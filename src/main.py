...
"""
python main.py --max_steps 10001 --valid_steps 1000
python main.py --max_steps 101 --valid_steps 10
"""
...
def parse_args(args=None):
    ...
    parser.add_argument('--do_valid', action='store_true', default=False, help="do valid")
    parser.add_argument('--do_test', action='store_true', default=True, help="do test")

    parser.add_argument('--data_path', type=str, default="/root/FuzzQE-code-master/data/drugbank", help="KG data path")
    ...
    parser.add_argument('-save', '--save_path', default='/root/FuzzQE-code-master/trained_models', type=str, help="no need to set manually, will configure automatically")
    parser.add_argument('--max_steps', default=10001, type=int, help="maximum iterations to train")
    ...
    parser.add_argument('--geo', default='vec', type=str, choices=['vec', 'box', 'beta', 'fuzzy'], help='the reasoning model, vec for GQE, box for Query2box, beta for BetaE')
    parser.add_argument('--print_on_screen', action='store_true')
    """
    Training query info:
        1p,2p,3p,2i,3i: 107982
        2in,3in,inp,pin,pni: 10798
    Validation query info:
        1p: 16910
        2p,3p,2i,3i,ip: 4000,pi,2in,3in,inp,pin,pni,2u-DNF,up-DNF: 4000
    Test query info:
        1p: 17021
        2p,3p,2i,3i,ip: 4000,pi,2in,3in,inp,pin,pni,2u-DNF,up-DNF: 4000
    """
    ...
    parser.add_argument('--tasks', default='1p.2i', type=str, help="tasks connected by dot, refer to the BetaE paper for detailed meaning and structure of each task")
    ...
    parser.add_argument('--continue_train', default='model_epoch.pt', type=str, help='run name to load and continue training')

    # gumbel softmax
    parser.add_argument('--gumbel_temperature', default=1, type=float, help="Gumbel temperature for gumbel softmax")
...
def main(args):
    ...
    if args.cuda:
        ...
        os.environ["CUDA_VISIBLE_DEVICES"] = '0' # args.gpu_ids  # specify which GPU(s) to be used
    ...
    # Model save path
    args.save_path = join('/root/FuzzQE-code-master/trained_models')
    ...
    if args.continue_train is not None: 
        ...
        model_path = join(args.save_path, saved_run_name)
    ...
    if args.do_train:
        ...
        for step in range(init_step, args.max_steps):
            ...
            if step % 100 == 0 and step > 0:
                save_path = os.path.join('/root/FuzzQE-code-master/trained_models/model_epoch.pt'.format(step))
            ...
            if step % args.valid_steps == 0 and step > 0:
                ...
                if args.do_test:
                    ...
                    if last_best_metric is None or test_all_metrics[early_stop_metric] > last_best_metric[early_stop_metric]:
                        ...
                        # save
                        if args.geo == 'fuzzy':
                            print('*'*30, 'Save model model_epoch_{}.pt at step{}'.format(step, step), '*'*30)
                            a = ('model_epoch_{}.pt'.format(step))
                            save_path = os.path.join('/root/FuzzQE-code-master/trained_models', f'{a}.pt')
                            # save_path = os.path.join('/root/FuzzQE-code-master/trained_models/model_epoch_{}.pt'.format(step))
                            torch.save(model, save_path)
                        else:  # baseline models. can only save model.state_dict
                            # save_dir = join(args.save_path, 'baselines', args.geo, str(run.name))
                            save_dir = join('/root/FuzzQE-code-master/trained_models', 'baselines', args.geo, 'model_epoch_{}.pt'.format(step))
